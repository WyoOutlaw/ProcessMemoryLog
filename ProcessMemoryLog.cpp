...
FILE *fp = NULL;
int output_format_csv = 0;
char *output_program_name = NULL;
...
static void PrintCsvForProgram(char *program)
{
	int idx;
	...
	if (0)
	{
		for (idx = 0; idx < MAX_PROCESSES; idx++)
		{
			...
			{
				printf("%s\n", Processes[idx].name);
				printf("code,data ro,rw,stack,reserved\n");

				for (int i = 0; i < Processes[idx].count; i++)
				{
					printf("%d,%d,%d,%d,%d\n", Processes[idx].code[i], Processes[idx].data[i], Processes[idx].rw[i], Processes[idx].stack[i], Processes[idx].reserved[i]);
				}
	...
	else
	{
		...
		{
			// Skip any empty slots
			if (strncmp(Processes[idx].name, program, strlen(program)) == 0)
				break;
		}
...
void Report()
{
	...
	{
		PrintCsvForProgram(output_program_name);
	}
	else
	{
		PrintProcessInfo();
...
{
	fprintf(stderr, "usage: %s <filename> [csv=PROG]\n", prog);
	fprintf(stderr, "  filename - is the files that is the log file\n");
	fprintf(stderr, "  csv - optional parameter the generates csv of all memory data for a program, \n");
	fprintf(stderr, "        where PROG is the name of the program of interest.\n");
	exit(-1);
}
...
int main(int argc, char *argv[])
{
	...
	else
	{
		...
		{
			ptr = &argv[2][0];
			if (strncmp(ptr, "csv", 3) == 0)
			{
				output_format_csv = 1;
				output_program_name = ptr + 4;
			}
		}
	}
...
//	Debug	printf("processed record %d\n", ++count);
	}
	fclose(fp);
...
>>>>>>> 2656bb53768c7d083fcf4c0b8aa1e2caa04ebb27
